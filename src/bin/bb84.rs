...
fn alice(n: usize, senders: (Sender<State>, Sender<bool>), recievers: (Receiver<bool>, Receiver<(usize, u8)>)) -> impl FnOnce() -> (bool, Option<Vec<u8>>) {
    move || {
        ...
        for _ in 0..n {
            ...
            let qubit = if basis {CROSS_BASIS[bit as usize].clone()} else {PLUS_BASIS[bit as usize].clone()};
            senders.0.send(qubit).unwrap();
            let correct = recievers.0.recv().unwrap() == basis;
            senders.1.send(correct).unwrap();
            if correct {
...
fn bob(n: usize, senders: (Sender<bool>,Sender<(usize, u8)>), recievers: (Receiver<State>, Receiver<bool>)) -> impl FnOnce() -> (bool, Option<Vec<u8>>) {
    move || {
        ...
        for _ in 0..n {
            ...
            let measured_bit: u8 = if random_sample <= prob_of_0 {0} else {1};
            
            senders.0.send(basis).unwrap();
            let correct = recievers.1.recv().unwrap();
            if correct {
                correct_bits.push(measured_bit);
            }
        }
