...
pub fn expected_value<const N: usize, F: Real>(observable: &Matrix<N,N,F>, state: &State<N,F>) -> Result<F,&'static str> {
    if !observable.is_hermitian() { return Err("Observables need to be Hermitian matrices.")}

    Ok(state.dot(&(observable * state)).r)
}   

pub fn variance<const N: usize, F: Real>(observable: &Matrix<N,N,F>, state: &State<N,F>) -> Result<F, &'static str> {
